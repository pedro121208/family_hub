...
app.post("/tarefas", async (req, res) => {
  try {
    await db.query("INSERT INTO rotina (tarefa, quem) VALUES (?, ?)", [
      req.body.tarefa,
      req.body.quem,
    ]);
    res.json({ ok: true });
...
app.put("/tarefas/:id", async (req, res) => {
  try {
    await db.query("UPDATE rotina SET tarefa = ?, quem = ? WHERE id = ?", [
      req.body.tarefa,
      req.body.quem,
      req.params.id,
    ]);
...
app.post("/eventos", async (req, res) => {
  try {
    await db.query("INSERT INTO eventos (titulo, data) VALUES (?, ?)", [
      req.body.titulo,
      req.body.data,
    ]);
    res.json({ ok: true });
...
app.put("/eventos/:id", async (req, res) => {
  try {
    await db.query("UPDATE eventos SET titulo = ?, data = ? WHERE id = ?", [
      req.body.titulo,
      req.body.data,
      req.params.id,
    ]);
